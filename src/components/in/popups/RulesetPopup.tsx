...
const RulesetPopup:React.FC<Props> = ({args, setArgs, setRulesetPopup}) => {
    ...
    const funcPreview = (repeat: number) => {
        ...
        for (let i = 0; i < repeat; i++) {
            ...
            buttons.push(
                <div className={`dark:border-slate-600 border-r border-y flex items-center justify-center text-sm ${isLastButton ? 'rounded-r-md' : 'rounded-y-md'}`} />
            );
        }
        return buttons;
    };
    ...
    function generateSelectedCode(selectedArgs: number, selectedFunction: string): JSX.Element {
        return (
            <span className='flex gap-5'>
                <button onClick={() => setArgs({ ...args, [selectedFunction]: selectedArgs !== 0 ? selectedArgs - 1 : selectedArgs })} className='border w-8 border-red-500 hover:border-red-600 text-red-600 font-bold rounded-md'>-</button>
                <div className='grid grid-cols-6'>
                    <h2 className='p-1 px-2 font-semibold bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 
                    border-slate-300 dark:border-slate-600 border-l border-y rounded-l-md'>
                        {selectedFunction}
                    </h2>
                    {funcPreview(5)}
                </div>
                <button onClick={() => setArgs({ ...args, [selectedFunction]: selectedArgs !== 12 ? selectedArgs + 1 : selectedArgs })} className='border w-8 border-green-500 hover:border-green-600 text-green-600 font-bold rounded-md'>+</button>
        ...
        <>
            ...
            <button onClick={() => setPage("function")} className='flex flex-col p-3 bg-white hover:bg-slate-50 dark:bg-[#2c3136] hover:dark:bg-[#3d4247] shadow-md rounded-md'>
                <div className='grid grid-cols-6'>
                    ...
                    <h2 className='p-1 px-2 font-semibold bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300
                     border-slate-300 dark:border-slate-600 border-l border-y rounded-l-md'>F0</h2>
                     {funcPreview(5)}
                </div>
            </button>
