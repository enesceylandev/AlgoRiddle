import { IconDefinition, faBan, faChevronRight, faPlay, faReply, faShare, faUpLong } from '@fortawesome/free-solid-svg-icons';
...
const MapSelectorPopup: React.FC<Props> = ({ setTutorialPopup }) => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setBgClass(prevClass => prevClass === 'bg-slate-200 dark:bg-slate-800' ? '' : 'bg-slate-200 dark:bg-slate-800');
  }, 1000);

      return () => clearInterval(interval);
  }, []);

  const ControlButton:React.FC<ControlButtonProps> = ({ text, active, animating, icon }) => (
    <div
  ...
  const showcase = (type: string) => {
    switch(type){
      ...
      <div className='grid grid-cols-3'>
        <h2 className='p-1 sm:px-2 px-1 font-semibold bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300
        border-slate-300 dark:border-slate-900 border-x border-y rounded-l-md'>F0</h2>
        <div className={`dark:border-slate-900 ${bgClass} border-r border-y flex items-center justify-center text-sm`}/>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50" onClick={() => setTutorialPopup(false)}>
      <div className="relative bg-[#F7F7FC] dark:bg-[#212529] p-4 rounded-xl shadow-md mx-5" onClick={(e) => e.stopPropagation()}>
        ...
        <ul className='text-slate-900 dark:text-slate-300 mb-5 list-decimal list-inside'>
          ...
          <li>Then, specify the action you want to apply in the selected slot from the commands module.</li>
          <li>Finally, when you feel ready, press the start button to play the solution you prepared.</li>
        </ul>
        <button
