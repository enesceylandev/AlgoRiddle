...
const Palette:React.FC<Props> = ({selectedBlock, setSelectedBlock, editorMap, setEditorMap, setPlayer}) => {
    ...
    const changeMap = (color: string, type:string, rotate?:string,) => {
        if(type === 'color'){
            if(color === 'blank'){
                setSelectedBlock?.([]);
  ...
  return (
    <div className='bg-slate-100 dark:bg-slate-700 w-[700px] p-2 rounded-md flex items-center justify-between z-40'>
        ...
        <button onClick={() => changeMap('faShuttleSpace', 'symbol', 'left')} 
            ...
            <button onClick={() => changeMap('faShuttleSpace', 'symbol', 'down')} 
            ...
            <button onClick={() => changeMap('faShuttleSpace', 'symbol', 'up')} 
            className='bg-blue-500 hover:bg-blue-600 text-white shadow-md rounded-md w-8 h-8'><FontAwesomeIcon icon={faShuttleSpace} className='-rotate-90'/></button>
            
            <button onClick={() => changeMap('faShuttleSpace','symbol', 'right')} 
            ...
            <button onClick={() => changeMap('faStar','symbol')} 
            className='bg-blue-500 hover:bg-blue-600 text-white shadow-md rounded-md w-8 h-8'><FontAwesomeIcon icon={faStar}/></button>

        </div>

        <div className='space-x-3 z-40'>
            <button onClick={() => changeMap('blue', 'color')} className='bg-blue-500 hover:bg-blue-600 text-white shadow-md rounded-md w-8 h-8'/>
            <button onClick={() => changeMap('purple', 'color')} className='bg-purple-500 hover:bg-purple-600 text-white shadow-md rounded-md w-8 h-8'/>
            <button onClick={() => changeMap('red', 'color')} className='bg-red-500 hover:bg-red-600 text-white shadow-md rounded-md w-8 h-8'/>
            <button onClick={() => changeMap('blank', 'color')} className='border text-white shadow-md rounded-md w-8 h-8'/>
        </div>
