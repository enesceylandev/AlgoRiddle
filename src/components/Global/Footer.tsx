...
const FooterLink: React.FC<{ to: string; text: string }> = ({ to, text }) => {
  return (
    <Link to={to} className='text-slate-600 dark:text-slate-400 hover:underline'>
      {text}
    </Link>
  );
};

const FooterSection: React.FC<{ title: string, children: ReactNode }> = ({ title, children }) => {
  return (
    <div className='lg:col-span-2 col-span-5 flex flex-col'>
      <h1 className='text-slate-900 dark:text-slate-100 text-xl font-semibold mb-2'>{title}</h1>
      {children}
    </div>
  );
};
    ...
    <div className='border-t dark:border-slate-800 w-full flex justify-center px-16 py-6'>
      <div className='lg:max-w-[1000px] w-full flex flex-col justify-between space-y-9'>
        ...
        <div className='sm:justify-end justify-center items-center flex text-xl space-x-5 text-slate-800 dark:text-slate-100'>
          ...
          <a href="mailto:enesfurkanceylan@hotmail.com" target="_blank" rel="noreferrer" className='p-2 rounded-md dark:hover:bg-slate-800 hover:bg-gray-200'>
